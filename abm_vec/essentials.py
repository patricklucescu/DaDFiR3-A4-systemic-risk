...
def compute_expected_supply_price(firm_ex_supply: ndarray,
                                  firm_supply: ndarray,
                                  firm_price: ndarray,
                                  market_price: float,
                                  firm_wage: ndarray,
    ...
    firm_price = np.where(statement_1 | statement_2, np.maximum(0.0 * firm_wage / firm_prod,
                                                                firm_price * (1 + price_adj * prc_excess_supply_factor)),
                          firm_price)
    firm_supply = np.where(~(statement_1 | statement_2), firm_supply * (1 + supply_adj * sup_excess_supply_factor),
                           firm_supply)
    ...
    return firm_price, firm_supply, firm_total_wage
