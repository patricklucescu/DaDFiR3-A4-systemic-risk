import pandas as pd
import os
from abm.firms import BaseFirm, Firm
from abm.banks import BaseBank, Bank
from abm.baseclass import BaseAgent
from abm.essentials import get_git_root_directory, compute_pd
import numpy as np
import random
...
def generate_random_entities(firms_ids: list,
    ...
    # load bank data
    bank_df = pd.read_excel(os.path.join(get_git_root_directory(), 'data/banks-swiss.xlsx'))
    ...
    base_firm.change_prob_excess_supply(calibration_variables['firm_init_excess_supply_prob'])
    ...
    for i in range(len(firms_ids)):
        firms[firms_ids[i]] = Firm(
            ...
            price=base_firm.market_price + np.random.normal(0, 0.01 * base_firm.market_price),
            wage=wage[i],
...
def generate_new_entities(new_bank_ids: list,
    ...
    average_tier_1 = np.mean([banks[bank_id].tier_1_cap for bank_id in banks.keys()
                              if banks[bank_id].tier_1_cap != '-'])
    std_tier_1 = np.std([banks[bank_id].tier_1_cap for bank_id in banks.keys()
                         if banks[bank_id].tier_1_cap != '-'])
    average_gross_loans = np.mean([banks[bank_id].gross_loans for bank_id in banks.keys()])
    std_tier_gross_loans = np.std([banks[bank_id].gross_loans for bank_id in banks.keys()])
    ...
    bank_tier_1 = list(np.random.normal(average_tier_1, std_tier_1 ** 0.1, len(new_bank_ids)))
    bank_gross_loans = list(np.random.normal(average_gross_loans, std_tier_gross_loans ** 0.1, len(new_bank_ids)))
    ...
    supply = [max(x, calibration_variables['firm_supply_scaling']) for x in
              np.random.normal(average_supply, std_supply, len(new_firm_ids))]
    excess_supply = [max(x, 0) for x in np.random.normal(average_ex_supply, std_ex_supply, len(new_firm_ids))]
    price = [max(x, base_firm.market_price / 2) for x in
             np.random.normal(average_price, std_price, len(new_firm_ids))]
    wage = [max(x, base_firm.min_wage) for x in np.random.normal(average_wage, std_wage, len(new_firm_ids))]
    firm_equity = [max(x, calibration_variables['firm_equity_scaling'] * 0.5) for x in
                   np.random.normal(average_firm_equity, std_firm_equity, len(new_firm_ids))]
