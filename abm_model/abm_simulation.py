    ...
    for firm_id in loan_clearing_order:
        ...
        for loan in loans_offered:
            ...
            if not bank_condition:
                ...
                counter_interbank_loan_request += 1
                ...
                loan_extended = True
                counter_interbank_loan_accepted += 1
            ...
            if loan_extended:
                ...
                # problem might arise since cds assignment
                if (covered_cds_prob > 0 or naked_cds_prob > 0):
                    #determine which banks want cds on this loan
                    interested_cds_buyers = [bank_id for bank_id in banks if (banks[bank_id].decide_cds(covered=(loan.lender == bank_id)))]
                    #for each interested buyer, get offers from various bank according to max_cds_requests, including only banks that are
                    #neither the lender bank itself nor any interested buyer (and neither the bank as its own counterparty)
                    cds_offers = [{bank_id_buyer : CDS(bank_id_buyer,counterparty_id,loan.borrower,loan.prob_default_borrower,
                    ...
                    cds_offers = {bank_id: sorted(cds_offers[bank_id], key=lambda y: y.spread) for bank_id in cds_offers}
                    ...
                    for bank_id in cds_transactions:
                        ...
                        logs.append(LogMessage(
                            message=f'CDS sold from {cds_transactions[bank_id].seller} to {cds_transactions[bank_id].buyer} on {cds_transactions[bank_id].reference_entity}',
                            time=t,
