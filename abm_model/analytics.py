...
def analytics(historic_data: dict,
    ...
    historic_data['banks_equity_by_time'], historic_data['banks_equity_by_bank'], historic_data['banks_debt_by_bank'] \
        = update_bank_equity_and_debt(historic_data['banks_equity_by_time'], historic_data['banks_equity_by_bank'],historic_data['banks_debt_by_bank'], banks, t)
    ...
    if t==(T-1):
        ...
        # plt.show()
        #
        # plt.plot(historic_data['bank_defaults'], color=color1)
        # plt.ylabel('Defaults', color=color1)
        # plt.tick_params(axis='y', labelcolor=color1)
        # rightAx = plt.twinx()
        # rightAx.plot(historic_data['firm_defaults'], color=color2)
        # rightAx.set_ylabel('Firm Defaults', color=color2)
        ...
        print(f'Last firm: {base_agent.firm_ids[len(base_agent.firm_ids)-1]}')
...
def update_bank_equity_and_debt(historic_bank_equity_by_time: dict,
                       historic_bank_equity_by_bank: dict,
                       historic_bank_debt_by_bank: dict,
                       banks: dict,
                       t: float) -> [dict,dict]:
    """
    ...
    :param: historic_bank_debt_by_bank: A dictionary containing the historic bank debt data sorted by bank.
    :param banks: A dictionary containing all banks.
    :param t: The current time step.
    ...
    updated_bank_debt_by_bank = historic_bank_debt_by_bank
    for bank_id in banks:
        if (bank_id not in updated_bank_equity_by_bank.keys()) or (t == 0):
            updated_bank_equity_by_bank.update({bank_id: {}})
            updated_bank_debt_by_bank.update({bank_id: {}})
        updated_bank_equity_by_bank[bank_id].update({t: banks[bank_id].equity})
        updated_bank_debt_by_bank[bank_id].update({t: banks[bank_id].deposits})

    return updated_bank_equity_by_time, updated_bank_equity_by_bank, updated_bank_debt_by_bank
