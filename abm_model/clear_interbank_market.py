...
def clear_interbank_market(
    ...
    for contract in interbank_contracts:
        if type(contract) == Loan:
            ...
            liabilities[buyer, seller] += contract.notional_amount * (
                1 + contract.interest_rate
            )
        else:
    ...
    initial_wealth = np.array(
        [
            banks[idx].equity
            + banks[idx].money_from_firm_loans
            + min([banks[idx].deposit_change, 0])
            for idx in banks_idx
        ]
    )
    while default:
    ...
    for bank_id, index in zip(banks_idx, range(len(banks_idx))):
        banks[bank_id].earnings = (
            banks[bank_id].equity
            + banks[bank_id].money_from_firm_loans
            + min([banks[bank_id].deposit_change, 0])
            + np.matmul(Pi.T, payments)[index]
            - payments[index]
        )
    ...
    for bank_id in banks_idx:
        money_for_deposits = (
            banks[bank_id].deposits
            - banks[bank_id].current_deposits
            - max([banks[bank_id].deposit_change, 0])
        )
        if bank_id in defaulted_banks or banks[bank_id].earnings < money_for_deposits:
            ...
            banks[bank_id].deposits = banks[bank_id].current_deposits + max(
                [banks[bank_id].deposit_change, 0]
            )
        else:
