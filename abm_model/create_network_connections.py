...
def create_network_connections(
    ...
    loan_clearing_order = random.sample(
        list(loan_offers.keys()), len(list(loan_offers.keys()))
    )
    ...
    for firm_id in loan_clearing_order:
        ...
        for loan in loans_offered:
            ...
            if (
                not bank_condition
            ):  # bank is above the maximum leverage if it accepts the loan
                continue
            elif (
                bank_condition > 0
            ):  # bank has enough money to extend the loan directly
                loan_extended = True
            else:
                credit_needed = -bank_condition
                interbank_loans = banks[loan.lender].get_potential_interbank_loans(
                    credit_needed, loan.notional_amount
                )
                interbank_loans = [
                    banks[bank_loan.lender].asses_loan_requests([bank_loan])
            ...
            if loan_extended:
                ...
                if covered_cds_prob > 0 or naked_cds_prob > 0:
                    ...
                    interested_cds_buyers = [
                        bank_id
                        for bank_id in banks
                        if (banks[bank_id].decide_cds(covered=(loan.lender == bank_id)))
                    ]
                    ...
                    interested_naked_cds_buyers = [
                        bank_id
                        for bank_id in interested_cds_buyers
                        if bank_id != loan.lender
                    ]
                    if len(interested_naked_cds_buyers) == (len(banks) - 1):
                        interested_cds_buyers = random.choices(
                            interested_naked_cds_buyers,
                            k=int(round(0.75 * len(banks), 0)),
                        )
                    ...
                    cds_offers = [
                        {
                            bank_id_buyer: CDS(
                                ...
                                loan.notional_amount * cds_fractional,
                                banks[counterparty_id].provide_cds_spread(loan),
                            )
                        }
                        for bank_id_buyer in interested_cds_buyers
                        for counterparty_id in random.choices(
                            [
                                x
                                for x in banks_idx
                                if (
                                    x != banks[bank_id_buyer].idx
                                    and x != loan.lender
                    ...
                    cds_offers = {
                        bank_id: sorted(cds_offers[bank_id], key=lambda y: y.spread)
                        for bank_id in cds_offers
                    }
                    # only consider the best offer (smallest spread)
                    cds_offers = {
                        bank_id: cds_offers[bank_id][0] for bank_id in cds_offers
                    }
                    # see if cds is affordable for buyer, add to transaction-list if so
                    cds_transactions = {
                        bank_id: cds_offers[bank_id]
                        for bank_id in cds_offers
                        if banks[bank_id].check_cds(
                            cds_offers[bank_id].spread
                    ...
                    for bank_id in cds_transactions:
                        banks[cds_transactions[bank_id].buyer].assets["cds"].append(
                            cds_transactions[bank_id]
                        )
                        banks[cds_transactions[bank_id].seller].liabilities[
                            "cds"
                        ].append(cds_transactions[bank_id])
